...
import {
  getPeriods,
  getTemplates,
  createEvaluationsFromTemplate,
  createSingleEvaluation,
} from "../services/evaluationService";
import { referenceService } from "../services/referenceService";
import type { User as UserType } from "../types/user";
import type { Period, Template, Criteria, TemplateCriteria, TemplateCriteriaByCategory } from "../types/evaluation";
...
interface CrearEvaluacionModalProps {
  ...
  onCreated: (
    newEvaluations:
      | { evaluatedEmployeeIds: number[]; count: number }
...
const CrearEvaluacionModal: React.FC<CrearEvaluacionModalProps> = ({
  ...
  // Fetch employees, references, periods, and templates
  useEffect(() => {
    if (show) {
      loadData();
    }
  ...
  const filteredEvaluatorUsers = useMemo(() => {
    return users.filter((user) => {
      ...
      const matchesSearch =
        ...
        (user.position || "")
          .toLowerCase()
          .includes(searchTermEvaluator.toLowerCase()) ||
        (user.department || "")
          .toLowerCase()
          .includes(searchTermEvaluator.toLowerCase());
  ...
  const validateForm = (): string | null => {
    ...
    if (selectedEmployees.length === 0)
      return "Seleccione al menos un empleado a evaluar";
    if (isSingleMode && selectedEmployees.length > 1)
      return "En modo individual, seleccione solo un empleado";
    return null;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSingleMode) {
        ...
        console.log(
          "🔄 Creating single evaluation:",
          JSON.stringify(payload, null, 2)
        );
        const data = await createSingleEvaluation(payload);
        console.log("✅ Single evaluation created:", data);
        onCreated(data);
        setConfirmationState({
          show: true,
          title: "¡Evaluación Creada!",
          message: `Se ha creado una evaluación con ID ${data.evaluation_id} exitosamente.`,
          type: "success",
          onConfirm: () =>
      ...
      console.error("❌ Error creating evaluation(s):", errorMessage);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-semibold text-gray-900">
                Crear {isSingleMode ? "Evaluación" : "Evaluaciones"}
              </h2>
              <p className="text-sm text-gray-600">
                Configure una nueva evaluación{" "}
                {isSingleMode ? "individual" : "masiva"}
              </p>
            </div>
          </div>
          <button
          ...
          {loadingData ? (
            ...
            <form onSubmit={handleSubmit} className="p-6">
              <div className="grid grid-cols-12 gap-6">
                      ...
                      {periods.map((period) => {
                        ...
                        return (
                          <option key={period.id} value={period.id}>
                            ...
                            {formatDate(period.due_date)}) - {status.label}
                          </option>
                        );
                      })}
                    </select>
                      ...
                      <div className="mt-4 space-y-2">
                        ...
                        {periods.slice(0, 3).map((period) => {
                          ...
                          return (
                            <div
                              key={period.id}
                              className={`flex items-center gap-2 p-2 rounded-lg ${status.bg} ${status.border} border text-xs`}
                            >
                              <StatusIcon
                  ...
                  <div className="bg-white border border-gray-200 rounded-xl p-6">
                    ...
                    >
                      ...
                      {templates.map((template) => {
                        const criteriaCount = template.criteria
                          ? Array.isArray(template.criteria)
                            ? template.criteria.length
                            : flattenTemplateCriteria(template.criteria).length
                          : 0;
                        return (
                          <option key={template.id} value={template.id}>
                            {template.name} ({criteriaCount} criterios)
                          </option>
                        );
                      })}
                    </select>
                    {templateId && (
                      <div className="mt-4">
                        ...
                        {(() => {
                          ...
                          const criteriaList = flattenTemplateCriteria(selectedTemplate.criteria);

                          return criteriaList.length > 0 ? (
                            criteriaList.map((criterion) => (
                              <div
                                key={criterion.id || criterion.criteria_id}
                                className="text-xs text-gray-600 mb-1"
                              >
                                - {criterion.criteria?.name || "Criterio sin nombre"} (
                                {criterion.category || criterion.criteria?.category || "Sin categoría"},{" "}
                        ...
                        <div>
                          <p className="font-medium text-blue-900 text-base">
                            {selectedEvaluator.name ||
                              `${selectedEvaluator.first_name || ""} ${
                                selectedEvaluator.last_name || ""
                              }`.trim()}
                          </p>
                ...
                {/* Right Column - Employees & Mode Toggle */}
                <div className="col-span-12 md:col-span-4 space-y-4">
                  {/* Mode Toggle */}
                  <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                    <h3 className="font-semibold text-gray-900 text-base mb-3">
                    ...
                    <div className="flex gap-4">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          setSelectedEmployees((prev) => prev.slice(0, 1));
                        }}
                        className={`flex-1 py-2 px-4 rounded-xl text-sm font-medium ${
                          isSingleMode
                            ? "bg-blue-500 text-white"
              ...
              <div className="flex gap-4 pt-6 mt-6 border-t border-gray-200">
                ...
                >
                  ...
                  {isSingleMode
                    ? "Evaluación"
                    : `${selectedEmployees.length} Evaluacion${
                        selectedEmployees.length !== 1 ? "es" : ""
                      }`}
                </button>
              </div>
            </form>
