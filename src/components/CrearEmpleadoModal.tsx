...
const CrearEmpleadoModal: React.FC<CrearEmpleadoModalProps> = ({ show, onClose, onCreated }) => {
  ...
  const [showSuccess, setShowSuccess] = useState(false); // ✅ Estado de éxito agregado
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ✅ Mostrar confirmación elegante en lugar de alert
      setShowSuccess(true);
      await onCreated();
      
      // Reset form después de un delay para que se vea el éxito
      setTimeout(() => {
        setForm({
          first_name: '',
          last_name: '',
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-md flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl p-6 w-full max-w-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        {/* ✅ Success State - igual que en CrearPeriodoModal */}
        {showSuccess ? (
          <div className="text-center py-8">
            <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
              <UserCheck className="w-8 h-8 text-green-600" />
            </div>
            <h3 className="text-xl font-semibold text-gray-900 mb-2">¡Empleado Creado!</h3>
            <p className="text-gray-600">
              El empleado <strong>{form.first_name} {form.last_name}</strong> se ha registrado exitosamente en el sistema.
            </p>
          </div>
        ) : (
          <>
